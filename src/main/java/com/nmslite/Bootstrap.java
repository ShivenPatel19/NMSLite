...
public class Bootstrap
{
    ...
    public static void main(String[] args)
    {
        try
        {
            ...
            loadConfiguration()
                ...
                .compose(Bootstrap::deployAllVerticles)
                .onSuccess(v ->
                {
                ...
                .onFailure(cause ->
                {
                    ...
                    cleanup()
                        ...
                        .onComplete(closeResult ->
                        {
                            ...
                            {
                                logger.error("Failed to close Vertx instance: {}", closeResult.cause().getMessage());
                            }
    ...
    private static Future<JsonObject> initializeDatabase(JsonObject config)
    {
        try
        {
            databaseInitializer = new DatabaseInitializer(config.getJsonObject("database", new JsonObject()));

            return databaseInitializer.initialize().compose(v -> Future.succeededFuture(config));
        }
        catch (Exception exception)
        {
    ...
    private static Future<Void> deployAllVerticles(JsonObject config)
    {
        try
        {
            ...
            var serverOptions = new DeploymentOptions().setConfig(config.getJsonObject("server", new JsonObject()));
            ...
            return vertx.deployVerticle(new ServerVerticle(), serverOptions)
                .compose(serverId ->
                {
                    ...
                    var pollingOptions = new DeploymentOptions().setConfig(config);
                ...
                .compose(pollingId ->
                {
                    ...
                    var discoveryOptions = new DeploymentOptions().setConfig(config.getJsonObject("discovery", new JsonObject()));
    ...
    private static Future<Void> cleanup()
    {
        try
        {
            ...
            return Future.join(cleanupFutures)
                ...
                .onFailure(cause ->
                {
                    logger.error("Some cleanup operations failed: {}", cause.getMessage());
    ...
    private static void setupWorkerExecutor(JsonObject config)
    {
        ...
        {
            logger.error("Error in setupWorkerExecutor: {}", exception.getMessage());
        }
    }
    ...
    private static Future<JsonObject> loadConfiguration()
    {
        ...
        try
        {
            ...
            retriever.getConfig()
                ...
                .onFailure(cause ->
                {
                    logger.error("Failed to load configuration from application.conf: {}", cause.getMessage());
        ...
        {
            logger.error("Failed to create configuration retriever: {}", exception.getMessage());
